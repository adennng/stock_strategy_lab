...
class SignalChatSession(BaseModel):
    thread_id: str
    created_at: str
    updated_at: str
    title: str | None = None
...
class SignalSessionManager:
    ...
    CLI 列表和恢复的轻量元数据，例如 thread_id、标题和当前加载的 run_state。
    """

    def __init__(self, config: AppConfig | None = None):
        self.config = config or load_app_config()
        self.workspace_dir = self.config.root_dir / "artifacts" / "signal_agent_workspace"
        self.checkpoint_dir = self.workspace_dir / "checkpoints"
        self.sessions_dir = self.workspace_dir / "sessions"
        self.checkpoint_path = self.checkpoint_dir / "signal_agent.sqlite"
    ...
    def new_thread_id(self) -> str:
        return f"signal-chat-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    def get_session(self, thread_id: str) -> SignalChatSession | None:
        payload = self._load_registry().get(thread_id)
        return SignalChatSession.model_validate(payload) if payload else None
    ...
    def latest_session(self) -> SignalChatSession | None:
        sessions = self.list_sessions(limit=1)
        return sessions[0] if sessions else None

    def list_sessions(self, *, limit: int | None = None) -> list[SignalChatSession]:
        items = [
            SignalChatSession.model_validate(value)
    ...
    def update_session(
        ...
        *,
        ...
        if existing:
            session = SignalChatSession.model_validate(existing)
        else:
