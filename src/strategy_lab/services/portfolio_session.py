...
class PortfolioChatSession(BaseModel):
    thread_id: str
    created_at: str
    updated_at: str
    title: str | None = None
...
class PortfolioSessionManager:
    """PortfolioAgent 终端会话注册表。"""

    def __init__(self, config: AppConfig | None = None):
        self.config = config or load_app_config()
        self.workspace_dir = self.config.root_dir / "artifacts" / "portfolio_agent_workspace"
        self.checkpoint_dir = self.workspace_dir / "checkpoints"
        self.sessions_dir = self.workspace_dir / "sessions"
        self.checkpoint_path = self.checkpoint_dir / "portfolio_agent.sqlite"
    ...
    def new_thread_id(self) -> str:
        return f"portfolio-chat-{datetime.now().strftime('%Y%m%d-%H%M%S-%f')}"
    ...
    def get_session(self, thread_id: str) -> PortfolioChatSession | None:
        payload = self._load_registry().get(thread_id)
        return PortfolioChatSession.model_validate(payload) if payload else None
    ...
    def latest_session(self) -> PortfolioChatSession | None:
        sessions = self.list_sessions(limit=1)
        return sessions[0] if sessions else None

    def list_sessions(self, *, limit: int | None = None) -> list[PortfolioChatSession]:
        items = [PortfolioChatSession.model_validate(value) for value in self._load_registry().values()]
    ...
    def update_session(
        ...
        *,
        ...
        if existing:
            session = PortfolioChatSession.model_validate(existing)
        else:
